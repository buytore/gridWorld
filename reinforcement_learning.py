# -*- coding: utf-8 -*-
from collections import defaultdict, Counter
import math
import os
import random
from utils import argmax
import numpy as np    #MWB - required for average & std dev


import pickle
try:
    import matplotlib.pyplot as plt
    import numpy
except:
    plt = None  # lint:ok
    numpy = None  # lint:ok


def make_at_least_n_times(optimistic_reward, min_n):
    def at_least_n_times_exploration(actions, utilities, temperature, action_counter):
        utilities = [utilities[x] for x in actions]
        for i, utility in enumerate(utilities):
            if action_counter[actions[i]] < min_n:
                utilities[i] = optimistic_reward
        d = dict(zip(actions, utilities))
        uf = lambda action: d[action]
        return argmax(actions, uf)

    return at_least_n_times_exploration


def boltzmann_exploration(actions, utilities, temperature, action_counter):
    '''returns an action with a probability depending on utilities and temperature'''

    utilities = [utilities[x] for x in actions]
    f = open('boltzmann_utilities.txt', 'ab')
    print >> f, utilities
    f.close() #- MWB

    temperature = max(temperature, 0.01)
    _max = max(utilities)
    _min = min(utilities)
    if _max == _min:                    #determines whether a random action should be taken...
        return random.choice(actions)

    utilities = [math.exp(((u - _min) / (_max - _min)) / temperature) for u in utilities]
    probs = [u / sum(utilities) for u in utilities]
    i = 0
    tot = probs[i]
    r = random.random()
    while i < len(actions) and r >= tot:
        i += 1
        tot += probs[i]
#    print (actions[i])
    return actions[i]


def epsilon_greedy_exploration(actions, utilities, temperature, action_counter, epsilon=0.01):
    '''returns an action depending on utilities and counts
       adoption of Travis DeWolf's work on RL
       https://studywolf.wordpress.com/category/learning/reinforcement-learning/
       Adapted by Mark Bannan to work in SimpleAI'''
    intentional = True
    #f = open('epsilon_utilities.txt', 'ab')     #file to capture utility values
    utilities = [utilities[a] for a in actions]  # utilities becomes prior counts of action
    #print >> f, utilities                       # Captures utility values for comparative
    #f.close()                                   #- MWB
    if random.random() < epsilon:
        action = random.choice(actions)          # random action if random number is less than epsilon
        intentional = False
    else:
        _max = max(utilities)                    # max value of utilities LIST
        count = utilities.count(_max)            # counts the maximum value in utilities LIST
        if count > 1:
            best = [i for i in range(len(actions)) if utilities[i] == _max]   # when q[i] = maxQ then assign to i to best LIST
            i = random.choice(best)
        else:
            i = utilities.index(_max)

        action = actions[i]
    # return action, intentional
    return action


def make_exponential_temperature(initial_temperature, alpha):
    '''returns a function = initial / exp(n * alpha)'''
    def _function(n):
        try:
            return initial_temperature / math.exp(n * alpha)  # init_temp/2.71828^(n*alpha)
        except OverflowError:
            return 0.01
    return _function


class PerformanceCounter(object):

    def __init__(self, learners, names=None):
        self.learners = learners
        for i, learner in enumerate(learners):
            self.update_set_reward(learner)
            learner.accumulated_rewards = []
            learner.known_states = []
            learner.temperatures = []
            if names is None:
                learner.name = 'Learner %d' % i
            else:
                learner.name = names[i]

    def update_set_reward(self, learner):
        def set_reward(reward, terminal=False):
            if terminal:
                if len(learner.accumulated_rewards) > 0:
                    learner.accumulated_rewards.append(learner.accumulated_rewards[-1] + reward)
                else:
                    learner.accumulated_rewards.append(reward)
                learner.known_states.append(len(learner.Q))
                learner.temperatures.append(learner.temperature_function(learner.trials))
            learner.old_set_reward(reward, terminal)
        learner.old_set_reward = learner.set_reward
        learner.set_reward = set_reward

    def _make_plot(self, ax, data_name):
        for learner in self.learners:
            data = numpy.array(getattr(learner, data_name))
            print data_name + " has minimum: " + str(min(data)) + " Max: " + str(max(data)) + " Average: " \
                  + str(np.mean(data)) + " Std Dev: " + str(np.std(data))
            ax.plot(numpy.arange(len(data)), data, label=learner.name)
        nice_name = data_name.replace('_', ' ').capitalize()
        ax.set_title(nice_name)
        ax.legend()

    def show_statistics(self):
        f, (ax1, ax2, ax3) = plt.subplots(3, 1, sharex=True)
        self._make_plot(ax1, 'accumulated_rewards')
        self._make_plot(ax2, 'known_states')
        self._make_plot(ax3, 'temperatures')
        plt.show()


class RLProblem(object):

    def actions(self, state):
        '''Returns the actions available to perform from `state`.
           The returned value is an iterable over actions.
        '''
        raise NotImplementedError()

    def update_state(self, percept, agent):
        'Override this method if you need to clean perception to a given agent'
        return percept


def inverse(n):
    if n == 0:
        return 1
    return 1.0 / n


def state_default():
    return defaultdict(int)


class QLearner(object):
    """
    QLearning class setups type of learning and type of exploration (default=boltzmann)
    """
    def __init__(self, problem, temperature_function=inverse, discount_factor=0.8, exploration_function=boltzmann_exploration, learning_rate=inverse):

        self.Q = defaultdict(state_default)
        self.problem = problem
        self.discount_factor = discount_factor
        self.temperature_function = temperature_function
        #self.exploration_function = boltzmann_exploration              ##############CHANGE EXPLORER##############
        self.exploration_function = epsilon_greedy_exploration          ##############CHANGE EXPLORER##############
        self.learning_rate = learning_rate

        self.last_state = None
        self.last_action = None
        self.last_reward = None
        self.counter = defaultdict(Counter)
        self.trials = 0

    def set_reward(self, reward, terminal=False):
        """sets rewards based on state = terminal or True"""
        self.last_reward = reward
        if terminal:
            self.trials += 1
            self.Q[self.last_state][self.last_action] = reward

    def program(self, percept):
        s = self.last_state
        a = self.last_action

        state = self.problem.update_state(percept, self)
        actions = self.problem.actions(state)

        if len(actions) > 0:
            current_action = self.exploration_function(actions, self.Q[state], self.temperature_function(self.trials), self.counter[state])
        else:
            current_action = None

        if s is not None and current_action:
            self.counter[s][a] += 1
            self.update_rule(s, a, self.last_reward, state, current_action)

        self.last_state = state
        self.last_action = current_action
        return current_action

    def update_rule(self, s, a, r, cs, ca):
        raise NotImplementedError

    def dump(self, path):
        self.temperature_function = inverse
        with open(path, 'wb') as f:
            pickle.dump(self, f)

    @classmethod
    def load(self, path):
        with open(path, 'rb') as f:
            return pickle.load(f)


class TDQLearner(QLearner):

    def update_rule(self, s, a, r, cs, ca):
        #f = open('tdq_learn_move.txt', 'ab')

        lr = self.learning_rate(self.counter[s][a])
        QMax = self.Q[cs].values()
        mark = max(QMax)
        self.Q[s][a] += lr * (r + self.discount_factor * max(self.Q[cs].values()) - self.Q[s][a])
        # Qt+1 := Qt+1 + alpha * [r + (gamma * max(Qt+1)) – Qt]
        #print >> f, "Action = " + a +  " !State= " + str(s) + " !CS = " + str(cs) + " !CA = " + ca + " !Reward = " + str(r)
        #f.close() #- MWB



class SARSALearner(QLearner):

    def update_rule(self, s, a, r, cs, ca):

        lr = self.learning_rate(self.counter[s][a])
        self.Q[s][a] += lr * (r + self.discount_factor * self.Q[cs][ca] - self.Q[s][a])
        # Qt+1 := Qt+1 + alpha * [r + (gamma * Qt+1) – Qt]